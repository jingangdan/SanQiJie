...
public class DayDayAfterPresenter extends MvpPresenter<DayDayAfterActivity> {
    public DayDayAfterPresenter(DayDayAfterActivity activity) {
        attachView(activity);
    }
    ...
    public void getCarLifeBanner(int action, String type) {
        Map<String, String> map = new HashMap<>();
        map.put("type", type);
        String time = System.currentTimeMillis() + "";
        try {
        ...
        map.clear();
        map.put("param", params);
        Observable<ResponseBody> typelist = service.getCarLifeBanner(map);
        loadData(action, typelist);
    }
