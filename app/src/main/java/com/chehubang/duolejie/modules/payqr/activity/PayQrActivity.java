...
import com.chehubang.duolejie.model.BrandMsg;
...
public class PayQrActivity extends BaseActivity<PayQrPresenter> implements MainView {
    ...
    private String username, header;
...
//        mDialog.show();
        ...
        if (s.equals("1")) {
            ...
            String pay_money = et_money.getText().toString().trim();
            startActivity(new Intent(this, PayResultActivity.class)
                    .putExtra("ussename", username)
                    .putExtra("header", header)
                    .putExtra("pay_money", pay_money));
//            startActivity(new Intent(this, PayResultActivity.class).putExtra("ussename", username).putExtra("header", header));
            this.finish();
        } else {
            mDialog = new ConfirmDialog(this);
            View content = LayoutInflater.from(this).inflate(R.layout.dialog_msg, null);
    ...
    private void bindBrandData(String s) {
        Log.e("ssssss", "datas = " + s);
        BrandMsg brandMsg = GsonUtil.gsonIntance().gsonToBean(s, BrandMsg.class);
        username = brandMsg.getGoodsBrandMap().getBrand_name();
        header = brandMsg.getGoodsBrandMap().getBrand_header();
        tv_store.setText("" + username);
//        try {
//            JSONObject obj = new JSONObject(s);
//            JSONObject brandMap = obj.getJSONObject("goodsBrandMap");
//            String brandName = brandMap.getString("brand_name");
//            username = brandName;
//            header = brandMap.getString("beand_header");
