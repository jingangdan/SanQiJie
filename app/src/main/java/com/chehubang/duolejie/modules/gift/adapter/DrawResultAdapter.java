    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (TextUtils.isEmpty(item.getCoupon_status())) {
            if ("y".equalsIgnoreCase(item.getStatus())) {
                holder.result.setText("免单：" + item.getCoupon_num() + " 元代金券");
            } else {
                holder.result.setText("继续努力哦");
            }
        } else {
            if ("进行中".equals(item.getCoupon_status())) {
                holder.result.setText("努力中");
            } else if ("倒计时".equals(item.getCoupon_status())) {
                holder.result.setText("努力中");
            } else {
                if ("y".equalsIgnoreCase(item.getStatus())) {
                    holder.result.setText("免单侠：" + item.getCoupon_num() + " 元代金券");
                } else {
                    holder.result.setText("继续努力哦");
                }
            }
